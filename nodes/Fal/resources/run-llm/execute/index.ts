...
import { RUN_LLM_MODEL_IDS, type RunLlmModelId } from '../models';
import { executeOpenRouter } from './openrouter';
...
> = {
	[RUN_LLM_MODEL_IDS.OPENROUTER]: executeOpenRouter,
};

/**
 * Router function for Run LLM operations
 */
export async function executeRunLlm(
	...
	if (operation === 'generate') {
		const modelId = context.getNodeParameter('model', itemIndex) as RunLlmModelId;
	...
	throw new NodeOperationError(
		context.getNode(),
		`The operation "${operation}" is not supported for Run LLM`,
		{ itemIndex },
	);
}
