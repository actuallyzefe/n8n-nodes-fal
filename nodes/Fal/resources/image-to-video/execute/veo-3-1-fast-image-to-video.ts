...
/**
 * Execute Google Veo 3.1 Fast Image-to-Video generation
 *
 * This model animates a single input image using Google's Veo 3.1 Fast model.
 * Faster variant with the same features as the standard Veo 3.1 Image-to-Video.
 */
export async function executeVeo31FastImageToVideo(
	context: IExecuteFunctions,
	itemIndex: number,
): Promise<IDataObject> {
	// Get model configuration
	const modelConfig = getModelConfig(IMAGE_TO_VIDEO_MODEL_IDS.VEO_3_1_FAST_IMAGE_TO_VIDEO);
